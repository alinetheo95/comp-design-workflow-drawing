...
(function() {
  ...
  // Add ambient light (reduced intensity for better reflections)
  const ambientLight = new THREE.AmbientLight(0xffffff, 0.3);
  scene.add(ambientLight);

  // Add main directional light (enhanced for better reflections)
  const directionalLight = new THREE.DirectionalLight(0xffffff, 1.2);
  directionalLight.position.set(5, 10, 7);
  directionalLight.castShadow = true;
  scene.add(directionalLight);

  // Add a second directional light for more dynamic reflections
  const directionalLight2 = new THREE.DirectionalLight(0x87ceeb, 0.7);
  directionalLight2.position.set(-5, 8, -3);
  scene.add(directionalLight2);

  // Add a third directional light from below for rim lighting
  const directionalLight3 = new THREE.DirectionalLight(0xffa500, 0.4);
  directionalLight3.position.set(0, -5, 5);
  scene.add(directionalLight3);

  // Add multiple point lights for complex reflections
  const pointLight1 = new THREE.PointLight(0xffffff, 0.8, 15);
  pointLight1.position.set(0, 8, 0);
  scene.add(pointLight1);

  const pointLight2 = new THREE.PointLight(0xff6b6b, 0.6, 12);
  pointLight2.position.set(4, 6, 4);
  scene.add(pointLight2);

  const pointLight3 = new THREE.PointLight(0x4ecdc4, 0.6, 12);
  pointLight3.position.set(-4, 6, -4);
  scene.add(pointLight3);

  const pointLight4 = new THREE.PointLight(0xfeca57, 0.5, 10);
  pointLight4.position.set(0, 4, -6);
  scene.add(pointLight4);

  // Add spot lights for dramatic highlights
  const spotLight1 = new THREE.SpotLight(0xffffff, 1.0, 20, Math.PI / 6, 0.3);
  spotLight1.position.set(8, 12, 8);
  spotLight1.target.position.set(0, 0, 0);
  scene.add(spotLight1);
  scene.add(spotLight1.target);

  const spotLight2 = new THREE.SpotLight(0x9966ff, 0.7, 18, Math.PI / 8, 0.2);
  spotLight2.position.set(-6, 10, 6);
  spotLight2.target.position.set(2, 0, -2);
  scene.add(spotLight2);
  scene.add(spotLight2.target);

  // Create a wireframe grid with enhanced material
  const gridHelper = new THREE.GridHelper(10, 20, 0x666666, 0x444444);
  scene.add(gridHelper);

  // Add a subtle reflective floor plane for enhanced reflections
  ...
  // Create materials from your color scheme with metallic properties
  const sphereMaterials = [];
  colorScheme.forEach(colorData => {
    const [r, g, b] = colorData.color;
    const material = new THREE.MeshStandardMaterial({ 
      color: new THREE.Color(r/255, g/255, b/255),
      metalness: 0.9,        // High metalness for realistic metal appearance
      roughness: 0.1,        // Low roughness for smooth, polished surface
